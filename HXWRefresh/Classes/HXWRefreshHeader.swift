...
extension HXWRefreshHeader {
    ...
    override public final func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if #available(iOS 11.0, *) {
            if ((offsetY < (headerHeight + (contentScrollview?.adjustedContentInset.top)!))&&(state != .normal)&&((contentScrollview?.isDragging)!)) {
                state = .normal
            }
        } else {
        ...
        if state == .normal {
            //设置拖动百分比
            if #available(iOS 11.0, *) {
                progress = (offsetY - (contentScrollview?.adjustedContentInset.top)!)/headerHeight
            } else {
